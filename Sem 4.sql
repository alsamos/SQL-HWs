...
-- Получите список пользователей, которые отправили сообщение 
SELECT u.*, m.* -- users.*, messages.*
FROM users u -- users = u, левая
JOIN messages m -- INNER JOIN = JOIN
ON u.id = m.from_user_id;


SELECT m.*, u.* -- users.*, messages.*
FROM messages m -- users = u, левая
LEFT JOIN users u-- INNER JOIN = JOIN
ON u.id = m.from_user_id;

SELECT 
	id,
    CONCAT(firstname,' ', lastname) AS "Пользователь", -- "Имя Фамилия"
    (SELECT user_id FROM profiles  WHERE user_id = users.id) AS "Внешний ключ", -- Если совпадает с первичным ключом, 
...
WHERE user_id = (SELECT id FROM users WHERE email = "arlo50@example.org")
AND media_type_id IN (
SELECT id FROM media_types WHERE name_type LIKE 'photo'); -- media_type_id = 2 OR media_type_id = 4  OR media_type_id
...
INNER JOIN users
	ON user_id = users.id
WHERE user_id = (SELECT user_id, YEAR(NOW()) - YEAR(birthday) AS "age"
FROM profiles
WHERE YEAR(NOW()) - YEAR(birthday) < 12 ON user_id = users.id;
