...
INSERT INTO users (login, pass, male) VALUES ('mila', '$3y$10$6SzbBCMENklStIgTqBKIluijJUnbeZ5WqIu4RJgkksnFZon5kH14y', 2);
...
SELECT title, full_text, user_id,
	(SELECT login FROM users WHERE users.id = posts.user_id) AS 'login'
FROM posts;
...
SELECT title, full_text, user_id
FROM posts
WHERE (SELECT login FROM users WHERE users.id = posts.user_id) = 'Mikle';